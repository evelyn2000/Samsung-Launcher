...
public class PagedViewCellLayout extends ViewGroup implements Page {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST) {
        	...
        	mPaddingLeft = 5;
            mPaddingRight = 5;
            newWidth = getResources().getDimensionPixelSize(R.dimen.widthspec_size);//(240, 217);
            newHeight = getResources().getDimensionPixelSize(R.dimen.hightspec_size);
            }
         ...
         */
        final int childMeasureWidth;
        final int childMeasureHeight;
        if(!com.mediatek.common.featureoption.FeatureOption.Qs_Sub_Project_Name.startsWith("A801")){
        	childMeasureWidth = Math.max(0, newWidth - mPaddingLeft - mPaddingRight);
        	childMeasureHeight = Math.max(0, newHeight - mPaddingTop - mPaddingBottom); 
        }else{
        	childMeasureWidth = getResources().getDimensionPixelSize(R.dimen.widthspec_size);
        	childMeasureHeight = getResources().getDimensionPixelSize(R.dimen.childhight_measurespec);
        }
    ...
    public int estimateCellHSpan(int width) {
        ...
        if (LauncherLog.DEBUG_LAYOUT) {
            LauncherLog.w(TAG, "(PagedViewCellLayout)estimateCellHSpan width = " + width
                    + ", availWidth = " + availWidth + ", n = " + n + ",mWidthGap = " + mWidthGap
    ...
    public int estimateCellVSpan(int height) {
        ...
        if (LauncherLog.DEBUG_LAYOUT) {
            LauncherLog.w(TAG, "(PagedViewCellLayout)estimateCellVSpan height = " + height
                    + ", availHeight = " + availHeight + ", n = " + n + ",mHeightGap = " + mHeightGap
    ...
    public void calculateCellCount(int width, int height, int maxCellCountX, int maxCellCountY) {
        ...
        if (LauncherLog.DEBUG_LAYOUT) {
            LauncherLog.w(TAG, "(PagedViewCellLayout)calculateCellCount width = " + width
