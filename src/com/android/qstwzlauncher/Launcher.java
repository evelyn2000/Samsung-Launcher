...
public final class Launcher extends Activity
    ...
    private static final int MENU_EDIT_PAGE = Menu.FIRST + 1;
    private static final int MENU_WALLPAPER_SETTINGS = MENU_EDIT_PAGE + 1;
    private static final int MENU_CREATE_FOLDER = MENU_WALLPAPER_SETTINGS + 1;
    private static final int MENU_SEARCH = MENU_CREATE_FOLDER + 1;
    ...
    private enum State { WORKSPACE, APPS_CUSTOMIZE, APPS_CUSTOMIZE_SPRING_LOADED, WORKSPACE_SPRING_LOADED, WORKSPACE_PREVIEW };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
        ...
        if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT){
        	...
        	boolean isTouchPenSlideOpen = intent.getBooleanExtra("qs_touchpen_slide_open", false);
            boolean isTouchPenSlideClose = intent.getBooleanExtra("qs_touchpen_slide_close", false);
			
            if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "Launcher::onCreate===isTouchPenSlideOpen:"+isTouchPenSlideOpen+"==isTouchPenSlideClose:"+isTouchPenSlideClose);
			}

            if(isTouchPenSlideOpen){
    			mWorkspace.showChildPenCellLayoutVisibility(true, true);
    		} else if(isTouchPenSlideClose){
    			mWorkspace.showChildPenCellLayoutVisibility(false);
    		}
        }

        // Update customization drawer _after_ restoring the states
        if(AppsCustomizePagedView.QS_FRAGMENT_APPS_WIDGETS && mAppsCustomizeContentWidget != null){
    ...
    public boolean checkTouchPenSlided(){
    	try{
    		if(Settings.System.getInt(getContentResolver(), "qs_touchpen_slide_status") > 0)
       		 return true;
    ...
    protected void onStop() {
        ...
        if(QS_SUPPORT_REMOVE_SCREENPAGE){
        	...
        	if(mWorkspace != null){
        		sharePref.edit().putInt(QS_WORKSPACE_SCREEN_COUNT, 
	...
	protected void onResume() {
		...
		if (mtkWidgetView != null) {
			((IMtkWidget) mtkWidgetView).onResumeWhenShown(mWorkspace
					.getCurrentPage());
			if (LauncherLog.DEBUG_SURFACEWIDGET) {
				LauncherLog.d(TAG_SURFACEWIDGET,
						"onResumeWhenShown: mtkWidgetView = " + mtkWidgetView);
			}
		}
		...
		// When we resume Launcher, a different Activity might be responsible
		// for the app
		...
		if (!mWorkspaceLoading) {
			...
			// We want to let Launcher draw itself at least once before we force
			// it to build
		...
		if (!cmpName.equals("none")) {
			if (mOrientationListener.canDetectOrientation()) {
				...
				Toast.makeText(this, R.string.orientation, Toast.LENGTH_SHORT)
						.show();
			}
		}
		...
		if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT){
			if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "Launcher::onResume===isTouchPenSlideOpen:"+checkTouchPenSlided()
            		+"==mWorkspace:"+mWorkspace.isChildPenCellLayoutVisibility());
			}
			
			if(this.checkTouchPenSlided() != mWorkspace.isChildPenCellLayoutVisibility()){
				if(checkTouchPenSlided()){
					mWorkspace.showChildPenCellLayoutVisibility(true, true);
				} else {
					mWorkspace.showChildPenCellLayoutVisibility(false);
				}
    ...
    */
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if(com.mediatek.common.featureoption.FeatureOption.Qs_Sub_Project_Name.startsWith("A801")
        		&& !isAllAppsVisible()){
        	String numstr = "";
        	...
        	if(!TextUtils.isEmpty(numstr)){
        		if(event.getRepeatCount() == 0){
		        	...
		        	intent.setData(Uri.parse("tel:"+numstr));
	    	...
	    	if(keyCode == KeyEvent.KEYCODE_VOLUME_MUTE){
	    		if(event.getRepeatCount() == 20){
	    			int nMode = mAudioManager.getRingerMode();
		    		if(AudioManager.RINGER_MODE_SILENT != nMode && AudioManager.RINGER_MODE_VIBRATE != nMode){
		    			mAudioManager.setRingerMode(AudioManager.RINGER_MODE_VIBRATE, (AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_VIBRATE));
		    		}else{
		    			mAudioManager.setRingerMode(AudioManager.RINGER_MODE_NORMAL, (AudioManager.FLAG_SHOW_UI | AudioManager.ADJUST_SAME));
		    		}
	    		}
	    		return true;
	     	}
        ...
        //add by syq start
    	if(QsSsDealWithVolumeKey(keyCode)){
    		return true;
    	}
        //add by syq end
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
    	if(com.mediatek.common.featureoption.FeatureOption.Qs_Sub_Project_Name.startsWith("A801")
    			&& !isAllAppsVisible()){
        	if(keyCode == KeyEvent.KEYCODE_Q || keyCode == KeyEvent.KEYCODE_A || keyCode == KeyEvent.KEYCODE_W
        			|| keyCode == KeyEvent.KEYCODE_E || keyCode == KeyEvent.KEYCODE_R || keyCode == KeyEvent.KEYCODE_S
        			|| keyCode == KeyEvent.KEYCODE_D || keyCode == KeyEvent.KEYCODE_F || keyCode == KeyEvent.KEYCODE_Z
        			|| keyCode == KeyEvent.KEYCODE_X || keyCode == KeyEvent.KEYCODE_C || keyCode == KeyEvent.KEYCODE_0)
        	{
        		return true;
        	}
        	if(keyCode == KeyEvent.KEYCODE_VOLUME_MUTE){
    ...
    private void restoreState(Bundle savedState) {
        if (LauncherLog.DEBUG || LauncherLog.QS_STYLE_I9300) {
        ...
        if (mAppsCustomizeTabHost != null) {
            ...
            if (curTab != null) {
                ...
                if(mAppsCustomizeContent.setContentType(
                        mAppsCustomizeTabHost.getContentTypeForTabTag(curTab))){
                	
                	mAppsCustomizeContent.setVisibility(View.VISIBLE);
                	
                	if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
                        LauncherLog.d(TAG, "(Launcher)restoreState: set app success===");
                    }
                	if(mAppsCustomizeContentWidget != null)
                		mAppsCustomizeContentWidget.setVisibility(View.INVISIBLE);
                }
                
                if(AppsCustomizePagedView.QS_FRAGMENT_APPS_WIDGETS && mAppsCustomizeContentWidget != null){
                	
                	if(mAppsCustomizeContentWidget.setContentType(
                            mAppsCustomizeTabHost.getContentTypeForTabTag(curTab))){
                    	
                		mAppsCustomizeContentWidget.setVisibility(View.VISIBLE);
                		if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
                            LauncherLog.d(TAG, "(Launcher)restoreState: set widget success===");
                        }
                		mAppsCustomizeContent.setVisibility(View.INVISIBLE);
                    }
                }
                
                mAppsCustomizeTabHost.setCurrentTabByTag(curTab);

                if(AppsCustomizePagedView.QS_FRAGMENT_APPS_WIDGETS 
                		&& mAppsCustomizeContentWidget != null
                		&& mAppsCustomizeContentWidget.getVisibility() == View.VISIBLE){
                ...
                if (state == State.APPS_CUSTOMIZE && mAppsCustomizeTabHost.getVisibility() != View.VISIBLE){
            ...
            if(AppsCustomizePagedView.QS_FRAGMENT_APPS_WIDGETS 
            	...
            	currentIndex = savedState.getInt("apps_customize_currentIndex_widgets");
...
//        if (mHotseat != null) {
//            mHotseat.setup(this, dragController);
//        }
        ...
        QsScreenIndicator indicator = (QsScreenIndicator) (mAllWidgetGroup.findViewById(R.id.paged_view_indicator));
        if(indicator != null)
        	indicator.initial(this, mWorkspace);
        
        indicator = (QsScreenIndicator) (mAppsCustomizeTabHost.findViewById(R.id.paged_view_indicator));
        if(indicator != null)
        	indicator.initial(this, mAppsCustomizeContent);
        
        if(AppsCustomizePagedView.QS_FRAGMENT_APPS_WIDGETS && mAppsCustomizeContentWidget != null){
        	indicator = (QsScreenIndicator) (mAppsCustomizeTabHost.findViewById(R.id.paged_view_indicator_widgets));
    ...
    private void completeAddShortcut(Intent data, long container, int screen, int cellX,
        ...
        if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT && (layout == null || layout.isAttrsLocked()))
        	return;
    ...
    public void onDetachedFromWindow() {
        ...
        mDragLayer.clearAllResizeFrames();
        if (LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.w(TAG, "Launcher::onDetachedFromWindow: === = ");
        }
        if (mAttached) {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_MAIN.equals(intent.getAction())) {
            ...
            boolean isTouchPenSlideOpen = Workspace.QS_SUPPORT_LOCK_CELLLAYOUT ? intent.getBooleanExtra("qs_touchpen_slide_open", false) : false;
            boolean isTouchPenSlideClose = Workspace.QS_SUPPORT_LOCK_CELLLAYOUT ? intent.getBooleanExtra("qs_touchpen_slide_close", false) : false;
            
            Folder openFolder = mWorkspace.getOpenFolder();
            // In all these cases, only animate if we're already on home
            mWorkspace.exitWidgetResizeMode();
            if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "Launcher::onNewIntent===isTouchPenSlideOpen:"+isTouchPenSlideOpen
            		+"==isTouchPenSlideClose:"+isTouchPenSlideClose);
			}
            if (alreadyOnHome && mState == State.WORKSPACE && !mWorkspace.isTouchActive() &&
                    openFolder == null) {
            	if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT && (isTouchPenSlideOpen || isTouchPenSlideClose)){
            ...
            } else if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT && (isTouchPenSlideOpen || isTouchPenSlideClose)){
            	if(isTouchPenSlideOpen){
        			mWorkspace.showChildPenCellLayoutVisibility(true, true);
        		} else if(isTouchPenSlideClose){
        			mWorkspace.showChildPenCellLayoutVisibility(false);
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.d(TAG, "(Launcher)onSaveInstanceState: mState = "+mState);
        }
    ...
    public void startSearch(String initialQuery, boolean selectInitialQuery,
        ...
        Rect sourceBounds = mSearchDropTargetBar == null ? null : mSearchDropTargetBar.getSearchBarBounds();
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(MENU_GROUP_WALLPAPER, MENU_EDIT_PAGE, 0, R.string.menu_edit)
            .setIcon(android.R.drawable.ic_menu_edit)
            .setAlphabeticShortcut('E');
        menu.add(MENU_GROUP_WALLPAPER, MENU_CREATE_FOLDER, 0, R.string.zzzz_ss_menu_create_folder)
        .setIcon(R.drawable.tw_action_bar_icon_add_folder_holo_dark)
        .setAlphabeticShortcut('F');
        menu.add(MENU_GROUP_WALLPAPER, MENU_SEARCH, 0, R.string.zzzz_ss_menu_search)
        ...
        menu.add(0, MENU_SYSTEM_SETTINGS, 0, R.string.menu_settings)
            .setIcon(R.drawable.tw_action_bar_icon_setting_holo_dark)
            .setIntent(settings)
...
//        }
    ...
    public void onBackPressed() {
        ...
        if (mState == State.APPS_CUSTOMIZE) {
            showWorkspace(true);
        } else if(mState == State.WORKSPACE_PREVIEW){
        	hidePreiews(true);
    ...
    private void handleFolderClick(FolderIcon folderIcon) {
        ...
        // add by syq for samsung start
        if(folderIcon.mFolder != null){
        	FolderEditText titleText = ((FolderEditText)folderIcon.mFolder.getEditTextRegion());
        	if(!info.title.equals(titleText.getText().toString()))
        		titleText.setText(info.title);
        }
        if(info.QsGetFolderInfoItemCounts() == 0){
        	showRenameDialog(info);
        	return;
        }
        // add by syq for samsung end
        if (!info.opened) {
    ...
    public boolean onLongClick(View v) {
        ...
        if (allowLongPress && !mDragController.isDragging()) {
            if (itemUnderLongClick == null) {
                ...
                QsShowHomeScreenChooser();//startWallpaper();                               
            } else {
            ...
            if (!TextUtils.isEmpty(name)) {
                ...
                if (mWorkspaceLoading) {
                    ...
                    if (folderIcon != null) {
                    	...
                    	folderIcon.onTitleChanged(name);//add by syq
        ...
        if (animated) {
            ...
            scaleAnim.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if (!LauncherLog.QS_STYLE_I9300 && !animationCancelled) {
...
//        		Animator animator = android.animation.AnimatorInflater.loadAnimator(this, R.animator.exit_home_fragment);
//                animator.setTarget(mAllWidgetGroup);
//                //animator.addListener(this);
//                arraylist.add(animator);
//                mAllWidgetGroup.setPivotX(mAllWidgetGroup.getWidth() / 2);
//                mAllWidgetGroup.setPivotY(0.0F);
//                
//                animator = android.animation.AnimatorInflater.loadAnimator(this, R.animator.enter_menu_darken);
//                animator.setTarget(mWorkspace);
//                arraylist.add(animator);
//                
...
//	            });
//                
//                mStateAnimation = animatorset;
//                
//                boolean delayAnim = false;
//	            if (toView instanceof LauncherTransitionable) {
//	                LauncherTransitionable lt = (LauncherTransitionable) toView;
//	                delayAnim = lt.onLauncherTransitionStart(instance, null, false);
//	            }
//	            // if the anim is delayed, the LauncherTransitionable is responsible for starting it
//	            if (!delayAnim) {
//	                // TODO: q-- what if this anim is cancelled before being started? or started after
...
//	            toView.setVisibility(View.VISIBLE);
//	            toView.setAlpha(0f);
//	            final ValueAnimator alphaAnim = ValueAnimator.ofFloat(0f, 1f).setDuration(fadeDuration);
//	            alphaAnim.setInterpolator(new DecelerateInterpolator(1.5f));
//	            alphaAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
//	                public void onAnimationUpdate(float a, float b) {
//	                    // don't need to invalidate because we do so above
...
//	            mStateAnimation = new AnimatorSet();
//	            //mStateAnimation.play(scaleAnim).after(startDelay);
//	            mStateAnimation.play(alphaAnim);
//        	
//	        	
//	            boolean delayAnim = false;
//	            if (toView instanceof LauncherTransitionable) {
...
//        		
//	            
//        		Animator animator = android.animation.AnimatorInflater.loadAnimator(this, R.animator.enter_home_fragment);
//                animator.setTarget(mAllWidgetGroup);
//                //animator.addListener(this);
//                arraylist.add(animator);
//                mAllWidgetGroup.setPivotX(mAllWidgetGroup.getWidth() / 2);
//                mAllWidgetGroup.setPivotY(0.0F);
//                
////                animator = android.animation.AnimatorInflater.loadAnimator(this, R.animator.exit_menu_darken);
////                animator.setTarget(mWorkspace);
////                arraylist.add(animator);
//                
//                final ValueAnimator alphaAnim = ValueAnimator.ofFloat(0f, 1f);
//	            alphaAnim.setDuration(res.getInteger(R.integer.config_appsCustomizeFadeOutTime));
//	            alphaAnim.setTarget(fromView);
//	            alphaAnim.setInterpolator(new AccelerateDecelerateInterpolator());
//	            alphaAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
//	                public void onAnimationUpdate(float a, float b) {
//	                    fromView.setAlpha(a * 1f + b * 0f);
...
//    }
    ...
    void showWorkspace(boolean animated) {
        ...
        if (mState != State.WORKSPACE) {
            ...
            //if(mState == )
            mWorkspacePreviewGroup.setVisibility(View.GONE);
    ...
    void enterSpringLoadedDragMode() {
    	if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.e(TAG, "enterSpringLoadedDragMode(0)===mState:"+mState);
        }
    	android.util.Log.w("QsLog", "enterSpringLoadedDragMode=========mState:"+mState);
        if (mState == State.APPS_CUSTOMIZE) {
        ...
        if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.e(TAG, "enterSpringLoadedDragMode(1)===mState:"+mState);
        }
    }
    ...
    void exitSpringLoadedDragModeDelayed(final boolean successfulDrop, boolean extendedDelay) {
    	...
    	if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.e(TAG, "exitSpringLoadedDragModeDelayed()===mState:"+mState
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
            	if(LauncherLog.DEBUG_QS_I9300){
                	LauncherLog.e(TAG, "exitSpringLoadedDragModeDelayed(1)===mState:"+mState
                			+"==successfulDrop:"+successfulDrop);
                }
                if (successfulDrop || LauncherLog.QS_STYLE_I9300) {
    ...
    void exitSpringLoadedDragMode() {
    	...
    	if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.e(TAG, "exitSpringLoadedDragMode(0)===mState:"+mState);
        }
        ...
        if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.e(TAG, "exitSpringLoadedDragMode(1)===mState:"+mState);
        }
    }
    ...
    void showHotseat(boolean animated) {
        if (!LauncherApplication.isScreenLarge()) {
            if (animated) {
                int duration = SearchDropTargetBar.getTransitionInDuration();
    ...
    void hideHotseat(boolean animated) {
        if (!LauncherApplication.isScreenLarge()) {
            if (animated) {
                int duration = SearchDropTargetBar.getTransitionOutDuration();
    ...
    public void startBinding() {
        ...
        int count = workspace.getEditScreenCount();
        for (int i = 0; i < count; i++) {
            // Use removeAllViewsInLayout() to avoid an extra requestLayout() and invalidate().
            final CellLayout layoutParent = (CellLayout) workspace.getChildAt(i);
            if(Workspace.QS_SUPPORT_LOCK_CELLLAYOUT && layoutParent.isAttrsLocked())
            	continue;
        ...
        mWidgetsToAdvance.clear();
        if (mHotseat != null) {
            mHotseat.resetLayout(this, mDragController);
        }
    }
    ...
    /* Cling related */
    private static final String PREFS_KEY = "com.android.qstwzlauncher.prefs";
            ...
            anim.addListener(new AnimatorListenerAdapter() {
                public void onAnimationEnd(Animator animation) {
                    ...
                    SharedPreferences prefs =
                        getSharedPreferences("com.android.qstwzlauncher.prefs", Context.MODE_PRIVATE);
	...
	void QsAddFolderByMenu(){
	    ...
	    long container = LauncherSettings.Favorites.CONTAINER_DESKTOP;//LauncherSettings.Favorites.CONTAINER_HOTSEAT
		addFolder(layout, container, screen, cellXY[0], cellXY[1]);
	}
	...
	AlertDialog mQsHomeScreenDlg;
	View.OnClickListener mQsHomeScreenOnClickListener = new View.OnClickListener() {
		@Override
		public void onClick(View v) {
			...
			if(mQsHomeScreenDlg != null)
				mQsHomeScreenDlg.dismiss();
		}
	};
	private void QsShowHomeScreenChooser(){
		if(mQsHomeScreenDlg == null){
			AlertDialog.Builder builder = new AlertDialog.Builder(this);
			builder.setTitle(R.string.zzzz_ss_long_click_dlg_title);
			LayoutInflater inflater = (LayoutInflater)getSystemService(Context.LAYOUT_INFLATER_SERVICE);
			LinearLayout view = (LinearLayout)inflater.inflate(R.layout.zzzz_ss_homescreen_chooser_dlg, null);
			QsHomeScreenChooserSetOnClick(view,"wallpaper");
			QsHomeScreenChooserSetOnClick(view,"app");
			QsHomeScreenChooserSetOnClick(view,"folder");
			QsHomeScreenChooserSetOnClick(view,"page");
			builder.setView(view);
			mQsHomeScreenDlg = builder.create();
		}
		mQsHomeScreenDlg.show();
	}
	private void QsHomeScreenChooserSetOnClick(ViewGroup group, String tag){
		View view = group.findViewWithTag(tag);
		if(view != null){
			view.setOnClickListener(mQsHomeScreenOnClickListener);
		}
	}
	
	//for volume dialog start
	SeekBar mQsCommonVolumeBar;
	SeekBar mQsNotificationVolumeBar;
	SeekBar mQsMusicVolumeBar;
	SeekBar mQsRingVolumeBar;
	ImageView mQsVolumeSettingBtn;
	ImageView mQsVolumeMainIcon;
	ImageView mQsVolumeRingIcon;
	ImageView mQsVolumeNotificationIcon;
	RelativeLayout mQsVolumeCommonContainer;
	LinearLayout mQsVolumeExpandContainer;
	OnSeekBarChangeListener mQsVolumeSeekBarChangeListener = new OnSeekBarChangeListener() {
		@Override
		...
		public void onProgressChanged(SeekBar seekBar, int progress,
			...
			int stream_type = AudioManager.STREAM_RING;
			String tag = (String)seekBar.getTag();
			if(tag.equals("seekbar_common")
				|| tag.equals("seekbar_ring")){
				stream_type = AudioManager.STREAM_RING;
			}else if(tag.equals("seekbar_music")){
				stream_type = AudioManager.STREAM_MUSIC;
			}else if(tag.equals("seekbar_notification")){
				stream_type = AudioManager.STREAM_NOTIFICATION;
			}
			mQsVolumeDlgHandler.removeMessages(QS_VOLUME_DLG_TIME_OUT_MSG);
			mQsVolumeDlgHandler.sendEmptyMessageDelayed(QS_VOLUME_DLG_TIME_OUT_MSG, QS_VOLUME_DLG_TIME_OUT);
			QsSsSetVolumeByTouch(progress, stream_type);
		}
	};
	
	AlertDialog mQsVolumeDialog;
	private static final int QS_VOLUME_DLG_TIME_OUT = 3000;
	private static final int QS_VOLUME_DLG_TIME_OUT_MSG = 1000;
	private Handler mQsVolumeDlgHandler = new Handler(){
		@Override
		public void handleMessage(Message msg) {
			if(msg.what == QS_VOLUME_DLG_TIME_OUT_MSG){
				if(mQsVolumeDialog != null){
					mQsVolumeDialog.dismiss();
				}
			}
		}
	};
	
	protected void QsSsShowVolumeDialog(boolean isUpVolume){
		if(mQsVolumeDialog == null){
			AlertDialog.Builder builder = new AlertDialog.Builder(Launcher.this);
			LayoutInflater inflater = (LayoutInflater)getSystemService(Context.LAYOUT_INFLATER_SERVICE);
			LinearLayout rootView = (LinearLayout)inflater.inflate(R.layout.zzzz_ss_volume_dlg, null);
			builder.setCustomTitle(null);
			builder.setView(rootView);
			mQsCommonVolumeBar = (SeekBar)rootView.findViewWithTag("seekbar_common");
			mQsNotificationVolumeBar = (SeekBar)rootView.findViewWithTag("seekbar_notification");
			mQsMusicVolumeBar = (SeekBar)rootView.findViewWithTag("seekbar_music");
			mQsRingVolumeBar = (SeekBar)rootView.findViewWithTag("seekbar_ring");
			mQsVolumeCommonContainer = (RelativeLayout)rootView.findViewWithTag("common_view");
			mQsVolumeExpandContainer = (LinearLayout)rootView.findViewWithTag("expand_view");
			mQsVolumeMainIcon = (ImageView)mQsVolumeCommonContainer.findViewWithTag("main_icon");
			mQsVolumeRingIcon = (ImageView)mQsVolumeExpandContainer.findViewWithTag("ring_icon");
			mQsVolumeNotificationIcon = (ImageView)mQsVolumeExpandContainer.findViewWithTag("notification_icon");			
			mQsVolumeSettingBtn = (ImageView)mQsVolumeCommonContainer.findViewWithTag("setting_btn");
			mQsVolumeSettingBtn.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					mQsVolumeDlgHandler.removeMessages(QS_VOLUME_DLG_TIME_OUT_MSG);
					mQsVolumeDlgHandler.sendEmptyMessageDelayed(QS_VOLUME_DLG_TIME_OUT_MSG, QS_VOLUME_DLG_TIME_OUT);
					mQsVolumeCommonContainer.setVisibility(View.GONE);
					mQsVolumeExpandContainer.setVisibility(View.VISIBLE);
				}
			});
			mQsVolumeDialog = builder.create();
			mQsVolumeDialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
				@Override
				public boolean onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
					if(event.getAction() == KeyEvent.ACTION_DOWN)
						return QsSsDealWithVolumeKey(keyCode);
					else
						return false;
				}
			});
			WindowManager.LayoutParams lp = mQsVolumeDialog.getWindow().getAttributes();
			lp.y = (int)(-175 * getResources().getDisplayMetrics().density);
			AudioManager am =(AudioManager)getSystemService(Context.AUDIO_SERVICE);

			mQsCommonVolumeBar.setMax(am.getStreamMaxVolume(AudioManager.STREAM_RING));
			mQsRingVolumeBar.setMax(am.getStreamMaxVolume(AudioManager.STREAM_RING));
			mQsMusicVolumeBar.setMax(am.getStreamMaxVolume(AudioManager.STREAM_MUSIC));
			mQsNotificationVolumeBar.setMax(am.getStreamMaxVolume(AudioManager.STREAM_NOTIFICATION));
			mQsCommonVolumeBar.setProgress(am.getStreamVolume(AudioManager.STREAM_RING));
			mQsRingVolumeBar.setProgress(am.getStreamVolume(AudioManager.STREAM_RING));
			mQsMusicVolumeBar.setProgress(am.getStreamVolume(AudioManager.STREAM_MUSIC));
			mQsNotificationVolumeBar.setProgress(am.getStreamVolume(AudioManager.STREAM_NOTIFICATION));
			mQsCommonVolumeBar.setOnSeekBarChangeListener(mQsVolumeSeekBarChangeListener);
			mQsRingVolumeBar.setOnSeekBarChangeListener(mQsVolumeSeekBarChangeListener);
			mQsMusicVolumeBar.setOnSeekBarChangeListener(mQsVolumeSeekBarChangeListener);
			mQsNotificationVolumeBar.setOnSeekBarChangeListener(mQsVolumeSeekBarChangeListener);
		}
		mQsVolumeCommonContainer.setVisibility(View.VISIBLE);
		mQsVolumeExpandContainer.setVisibility(View.GONE);
		QsSsSetVolumeByKey(isUpVolume);
		mQsVolumeDialog.show();
		mQsVolumeDlgHandler.sendEmptyMessageDelayed(QS_VOLUME_DLG_TIME_OUT_MSG, QS_VOLUME_DLG_TIME_OUT);
	}
	
	public void QsSsSetVolumeByKey(boolean isUpVolume){
		AudioManager am =(AudioManager)getSystemService(Context.AUDIO_SERVICE);
		int stream_type = AudioManager.STREAM_RING;
		int maxVolume = am.getStreamMaxVolume(stream_type);
		int current = am.getStreamVolume(stream_type);
		if(isUpVolume){
			QsSsSetVolume(Math.min(current + 1, maxVolume), true, stream_type);
		}else{
			QsSsSetVolume(Math.max(current - 1, 0), true, stream_type);
		}
	}
	
	private void QsSsSetVolume(int value, boolean isUpVolume, int streamType){
		AudioManager am =(AudioManager)getSystemService(Context.AUDIO_SERVICE);
		int mode = am.getRingerMode();
		int stream_type = streamType;
		int maxVolume = am.getStreamMaxVolume(stream_type);
		int current = am.getStreamVolume(stream_type);
		if(isUpVolume){
			int flagPlay = AudioManager.FLAG_PLAY_SOUND;
			if(current == 0 && stream_type == AudioManager.STREAM_RING){
				if(mode == AudioManager.RINGER_MODE_VIBRATE ||
		        	mode == AudioManager.RINGER_MODE_SILENT){
		        	am.setRingerMode(AudioManager.RINGER_MODE_NORMAL);
		        	flagPlay |= AudioManager.FLAG_VIBRATE;
		        }
			}
			am.setStreamVolume(stream_type, Math.min(value, maxVolume), flagPlay);
		}else{
			int flagPlay = AudioManager.FLAG_PLAY_SOUND;
			if(value == 0 && stream_type == AudioManager.STREAM_RING){
	        ...
	        am.setStreamVolume(stream_type, Math.max(value, 0), flagPlay);
		}
		QsSsUpdateVolumeIcon(am.getRingerMode());
		if(stream_type == AudioManager.STREAM_RING){
			mQsCommonVolumeBar.setProgress(am.getStreamVolume(stream_type));
			mQsRingVolumeBar.setProgress(am.getStreamVolume(stream_type));
		}
	}
	
	private void QsSsUpdateVolumeIcon(int mode){
		if(mode == AudioManager.RINGER_MODE_NORMAL){
			mQsVolumeRingIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_phone);
			mQsVolumeMainIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_phone);
			mQsVolumeNotificationIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_notification);
		}else if(mode == AudioManager.RINGER_MODE_SILENT){
			mQsVolumeRingIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_ring_notif_mute);
			mQsVolumeMainIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_ring_notif_mute);
			mQsVolumeNotificationIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_notification_mute);
		}else if(mode == AudioManager.RINGER_MODE_VIBRATE){
			mQsVolumeRingIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_ring_notif_vibrate);
			mQsVolumeMainIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_ring_notif_vibrate);
			mQsVolumeNotificationIcon.setImageResource(R.drawable.zzzz_ss_ic_audio_notification_mute);
		}
	}
	
	public void QsSsSetVolumeByTouch(int value, int type){
		AudioManager am =(AudioManager)getSystemService(Context.AUDIO_SERVICE);
		int stream_type = type;
		int currentValume = am.getStreamVolume(stream_type);
		if(currentValume == value)
			return;
		QsSsSetVolume(value, value > currentValume ? true : false, stream_type);
	}
	
	private boolean QsSsDealWithVolumeKey(int keyCode){
        if(keyCode == KeyEvent.KEYCODE_VOLUME_UP){
        	if(mQsVolumeDialog != null && mQsVolumeDialog.isShowing()){
        		QsSsSetVolumeByKey(true);
        	}else{
        		QsSsShowVolumeDialog(true);
        	}
        	return true;
        }else if(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN){
        	if(mQsVolumeDialog != null && mQsVolumeDialog.isShowing()){
        		QsSsSetVolumeByKey(false);
        	}else{
        		QsSsShowVolumeDialog(false);
        	}
        	return true;
        }
        return false;
	}
	//for volume dialog end
}
