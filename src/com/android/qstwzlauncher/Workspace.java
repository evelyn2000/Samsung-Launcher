...
import com.android.qstwzlauncher.FolderIcon.FolderRingAnimator;
import com.android.qstwzlauncher.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class Workspace extends SmoothPagedView
        ...
        DragController.DragListener, CellLayout.QsChildVisibilityChanged {
    ...
    {
        mBackgroundDarken = f;
        //mHomeFragment.getDarkenView().setAlpha(mBackgroundDarken);
    }
    
    public float getBackgroundDarken()
    {
        return mBackgroundDarken;
    }
    ...
    public final static boolean QS_SUPPORT_LOCK_CELLLAYOUT = false;
    private int mLockedScreenCount = 0;
    private ArrayList<View> mInVisibilityChildList = new ArrayList<View>();
    
//    protected float mSecondaryDownMotionX;
//    protected float mSecondaryDownMotionY;
    ...
    public Workspace(Context context, AttributeSet attrs, int defStyle) {
        ...
        mQsMaxScreenCount = a.getInt(R.styleable.Workspace_maxScreenCount, 5);
        ...
        int nChildCount = sharePref.getInt(Launcher.QS_WORKSPACE_SCREEN_COUNT, 
        		mQsDefaultScreenCount);
        ...
        //mQsDefaultScreenCount = getChildCount();
        ...
        if(QS_SUPPORT_LOCK_CELLLAYOUT){
        	//final View child = inflater.inflate(R.layout.workspace_screen_pen, this, false);
        	//((CellLayout)child).setQsChildVisibilityChanged(this);
        	//addView(child);
        }
    ...
    public boolean isChildPenCellLayoutVisibility(){
    	if(QS_SUPPORT_LOCK_CELLLAYOUT && getLockedScreenCount() > 0){
    		int index = getChildCount()-1;
    		CellLayout child = (CellLayout)getChildAt(index);
    		if(child.isAttrsLocked()){
    ...
    public void showChildPenCellLayoutVisibility(boolean show){
    	showChildPenCellLayoutVisibility(show, false);
    }
    
    public void showChildPenCellLayoutVisibility(boolean show, boolean selectedit){
    	if(QS_SUPPORT_LOCK_CELLLAYOUT && getLockedScreenCount() > 0){
    		int index = getChildCount()-1;
    		CellLayout child = (CellLayout)getChildAt(index);
    		if(child.isAttrsLocked()){
    			//show = (child.getVisibility() == View.GONE);
    			
    			int visiable = show ? View.VISIBLE : View.GONE;
    			if(visiable != child.getVisibility()){
	    			if(mQsWorkspaceCallback != null){
	    				if(show){
	    					mQsWorkspaceCallback.setCustomPageIndicatorIcon(QsScreenIndicatorCallback.CUSTOM_INDICATOR_LAST, 
	    							getResources().getDrawable(R.drawable.pageindicator_custom_pen));
	    				} else {
	    					mQsWorkspaceCallback.setCustomPageIndicatorIcon(QsScreenIndicatorCallback.CUSTOM_INDICATOR_LAST, 
	        						null);
	    				}
    ...
    public void onQsChildVisibilityChanged(View child, int visibility){

    	int oldsize = mInVisibilityChildList.size();
    	if(visibility == View.GONE){
    		if(!mInVisibilityChildList.contains(child))
    			mInVisibilityChildList.add(child);
    		int index = indexOfChild(child);
    		if(getCurrentPage() == index){
    			setCurrentPage(index-1);
    		}
    		
    	} else if(mInVisibilityChildList.contains(child)) {
    		mInVisibilityChildList.remove(child);
    	}
    	
    	if(mQsWorkspaceCallback != null && oldsize != mInVisibilityChildList.size())
        	mQsWorkspaceCallback.onPageCountChanged(getPageCount());
    	
    	if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "onQsChildVisibilityChanged()===visibility:"+visibility
    			+"==newsize:"+mInVisibilityChildList.size()
    			+"==oldsize:"+oldsize
    ...
    public void setOnLongClickListener(OnLongClickListener l) {
        ...
        for (int i = 0; i < count; i++) {
        	...
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && layout.isAttrsLocked()){
        		continue;
        	}
...
//    	final CellLayout cl = (CellLayout)super.getNextPageView();
...
//    }
    ...
    public int getLockedScreenCount(){
    	return mLockedScreenCount;
    }
    
    public int getEditScreenCount(){
    	return getChildCount() - getLockedScreenCount();
    }
    ...
    private View addScreen(LayoutInflater inflater, int index, boolean changeDefautlSize){
    	...
    	if(QS_SUPPORT_LOCK_CELLLAYOUT){
    		if(getLockedScreenCount() > 0){
	    		int childCount = getChildCount();
	    		if(index < 0 || index >= childCount){
	    			for(int i=childCount-1; i>=0; i--){
	    				final CellLayout cl = (CellLayout)getChildAt(i);
	        			if(cl != null && cl.isAttrsLast()){
	        				index = i;
	        			} else {
	        				break;
	        			}
    ...
    public boolean removeScreen(final int index, final boolean changeDefautlSize){
    	...
    	if(index >= 0 && index < getEditScreenCount() && getEditScreenCount() > getMinScreenCount()){
    		...
    		if(((CellLayout)view).isAttrsLocked())
    			return false;
    ...
    private void removeChildWidgets(CellLayout view){
    	...
    	CellLayoutChildren cell = view.getChildrenLayout();
    	if(cell != null){
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	...
    	if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "onDragStart()======mDragTargetLayout:"+(mDragTargetLayout == null ? "null" : mDragTargetLayout.isAttrsLocked())
        		);
		}
        ...
        for (int i = 0; i < getPageCount(); i++) {
        	final CellLayout cl = (CellLayout)getChildAt(i);
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && cl.isAttrsLocked())
        		continue;
    ...
    public void onDragEnd() {
        ...
        for (int i = 0; i < getPageCount(); i++) {
            ((CellLayout)getChildAt(i)).onDragEnd();
        }
    ...
    protected int getScrollMode() {
    	if(PagedView.QS_SUPPORT_LOOP_SLIDE)
    ...
    public void onChildViewAdded(View parent, View child){
        ...
        //android.util.Log.i("QsLog", "Workspace::onChildViewAdded()===Visibility:"+child.getVisibility()+"==mInVisibilityChildCount:"+mInVisibilityChildCount);
...
//    }
 ...
 //       android.util.Log.w("QsLog", "onTouch()===="+event.getSize()+"=="+event.getPointerCount());
...
//                		+"==mDownMotionDelta:"+mDownMotionDelta);
    ...
    protected boolean isScrollingIndicatorEnabled() {
        return (getScrollingIndicator() != null && mState != State.SPRING_LOADED);
    }
    ...
    protected void onPageBeginMoving() {
        ...
        if(LauncherLog.QS_STYLE_I9300 && !mIsDragOccuring){
        ...
        if(LauncherLog.QS_STYLE_I9300){
        	...
        	if (mScrollMode == X_LARGE_MODE) {
	            ...
	            if (!scrollComputed && mTouchState == TOUCH_STATE_SCROLLING) {
	            	...
	            	//android.util.Log.e("QsLog", "Workspace::computeScroll()======mNextPage:"+mNextPage);
	            	
	                final float now = System.nanoTime() / NANOTIME_DIV;
	                final float e = (float) Math.exp((now - mSmoothingTime) / SmoothPagedView.SMOOTHING_CONSTANT);
	                ...
	                if(!mIsDragOccuring){
		                CellLayout curCell = (CellLayout)getCurrentPageView();
		                if(curCell != null){
		                	float deltaX = (mLastMotionX - mDownMotionX);
		                	float percent = Math.abs(deltaX / curCell.getWidth());
		                	if(deltaX < 0.0f){
		                		
		                		CellLayout nextCell = (CellLayout)getNextPageView();
		                		
		                		curCell.setOverScrollAmount(percent, CellLayout.SCROLL_TO_LEFT);
		                		if(nextCell != null)
		                			nextCell.setOverScrollAmount(percent, CellLayout.SCROLL_TO_RIGHT);
		                		
		                	} else if(deltaX > 0.0f) {
		                		
		                		curCell.setOverScrollAmount(percent, CellLayout.SCROLL_TO_RIGHT);
		                		
		                		CellLayout prevCell = (CellLayout)getPreviousPageView();
		                		if(prevCell != null)
		                			prevCell.setOverScrollAmount(percent, CellLayout.SCROLL_TO_LEFT);
		                		
		                	} else {
		                		
		                		curCell.setOverScrollAmount(percent, CellLayout.SCROLL_TO_NONE);
	            ...
	            } else if(!mIsDragOccuring && mScroller.computeScrollOffset()){
	            	//mScroller.get
	            	CellLayout curCell = (CellLayout)getCurrentPageView();

                	float deltaX = (mScroller.getFinalX() - mTouchX);
                	float percent = Math.abs(deltaX / (mScroller.getFinalX() - mScroller.getStartX()));
                	if(deltaX < 0.0f){
    ...
    private void screenScrolledLargeUI(int screenCenter) {
    	...
    	if (LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.d(TAG, "screenScrolledLargeUI()===mOverScrollX:"+mOverScrollX
    ...
    private void screenScrolledStandardUI(int screenCenter) {
    	...
    	if (LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.d(TAG, "screenScrolledStandardUI()===mOverScrollX:"+mOverScrollX
            		+"==mMaxScrollX:"+mMaxScrollX);
        }
    ...
    protected void screenScrolled(int screenCenter) {
    	if (LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.d(TAG, "screenScrolled()====isScreenLarge:"+LauncherApplication.isScreenLarge());
        }
    ...
    protected void overScroll(float amount) {
    	if (LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.d(TAG, "overScroll()====amount:"+amount);
        }
        if (LauncherApplication.isScreenLarge()) {
    ...
    protected void onAttachedToWindow() {
        ...
        if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.w(TAG, "onAttachedToWindow: mWindowToken = " + mWindowToken);
        }
    ...
    protected void onDetachedFromWindow() {
        ...
        if (LauncherLog.DEBUG || LauncherLog.DEBUG_QS_I9300) {
            LauncherLog.w(TAG, "Workspace::onDetachedFromWindow: mWindowToken = " + mWindowToken);
        }
        mWindowToken = null;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBackground != null && mBackgroundAlpha > 0.0f && mDrawBackground) {
            ...
            //android.util.Log.w("QsLog", "workspace::onDraw()==alpha:"+alpha+"==mBackgroundAlpha:"+mBackgroundAlpha);
    ...
    void enableChildrenCache(int fromPage, int toPage) {
        ...
        //android.util.Log.w("QsLog", "Workspace::enableChildrenCache()==fromPage:"+fromPage+"==toPage:"+toPage);
    ...
    void clearChildrenCache() {
    	//android.util.Log.w("QsLog", "Workspace::clearChildrenCache()==");
        final int screenCount = getChildCount();
    ...
    */
    ...
    private void initAnimationArrays() {
        final int childCount = Math.max(getMaxScreenCount(), getChildCount());
    ...
    void changeState(final State state, boolean animated, int delay) {
    	...
    	if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.w(TAG, "changeState(0)===mState:"+mState+"==newstate:"+state+"==animated:"+animated);
        }
        ...
        if(LauncherLog.DEBUG_QS_I9300){
        	LauncherLog.w(TAG, "changeState(1)===stateIsNormal:"+stateIsNormal
        			+"==stateIsSpringLoaded:"+stateIsSpringLoaded
        			+"==stateIsSmall:"+stateIsSmall
        			+"==oldStateIsNormal:"+oldStateIsNormal
        			+"==oldStateIsSmall:"+oldStateIsSmall);
        }
    ...
    public boolean acceptDrop(DragObject d) {
        ...
        if (d.dragSource != this) {
            ...
            if (mDragTargetLayout == null || (QS_SUPPORT_LOCK_CELLLAYOUT && mDragTargetLayout.isAttrsLocked())) {
            	if (LauncherLog.DEBUG_QS_I9300) {
            		LauncherLog.d(TAG, "acceptDrop(1)===return false===");
				}
                return false;
            }
        	...
        	if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "acceptDrop(3)===source not workspace===TargetLayout:"+(mDragTargetLayout == null ? "null" : mDragTargetLayout.isAttrsLocked()));        	
			}
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && mDragTargetLayout != null && mDragTargetLayout.isAttrsLocked())
        		return false;
        }
        return true;
    }
    ...
    boolean willCreateUserFolder(ItemInfo info, CellLayout target, int[] targetCell,
        ...
        if(true){
//        	boolean aboveShortcut = (dropOverView.getTag() instanceof FolderInfo);
//	        boolean willBecomeShortcut =
//	                (info.itemType == LauncherSettings.Favorites.ITEM_TYPE_FOLDER ||
//	                info.itemType == LauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER);
	        
	        return false;//(aboveShortcut && willBecomeShortcut);
    ...
    public void onDrop(DragObject d) {
        ...
        if(QS_SUPPORT_LOCK_CELLLAYOUT && dropTargetLayout != null && dropTargetLayout.isAttrsLocked()){
        	return;
        }
            ...
            if (dropTargetLayout != null) {
                ...
                if (LauncherLog.DEBUG_DRAG) {
                    LauncherLog.d(TAG, "onDrop 2: cell = " + cell + ",screen = " + screen
                    		+",container="+container
                    		+",hasMovedLayouts="+hasMovedLayouts
                ...
                if (mTargetCell[0] >= 0 && mTargetCell[1] >= 0) {
                    ...
                    if (LauncherLog.DEBUG_DRAG) {
                        LauncherLog.w(TAG, "onDrop 4: item = " + info
                        		+",container="+container
                        		+",screen="+screen
                        		+",cellX="+lp.cellX
                                + ",cellY = " + lp.cellY );
                    ...
                    if (LauncherLog.DEBUG_DRAG) {
                        LauncherLog.w(TAG, "onDrop 5: item = " + info
    ...
    public void onDragEnter(DragObject d) {
        ...
        if(QS_SUPPORT_LOCK_CELLLAYOUT && layout != null && layout.isAttrsLocked())
        	return;
    ...
    public boolean onDragEvent(DragEvent event) {
        ...
        case DragEvent.ACTION_DRAG_STARTED: {
        	...
        	if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "onDragEvent(ACTION_DRAG_STARTED)======layout:"+(layout == null ? "null" : layout.isAttrsLocked()));
			}
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && layout != null && layout.isAttrsLocked())
        		return false;
            // Validate this drag
        ...
        case DragEvent.ACTION_DROP: {
            ...
            if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "onDragEvent(ACTION_DROP)======layout:"+(layout == null ? "null" : layout.isAttrsLocked()));
			}
			if(QS_SUPPORT_LOCK_CELLLAYOUT && layout != null && layout.isAttrsLocked())
        		return false;
        ...
        case DragEvent.ACTION_DRAG_ENDED:
        	...
        	if (LauncherLog.DEBUG_QS_I9300) {
            	LauncherLog.d(TAG, "onDragEvent(ACTION_DRAG_ENDED)======layout:"+(layout == null ? "null" : layout.isAttrsLocked()));
			}
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && layout != null && layout.isAttrsLocked())
        		return false;
...
//    	}
    ...
    public boolean onEnterScrollArea(int x, int y, int direction) {
    	...
    	android.util.Log.w("QsLog", "Workspace::onEnterScrollArea()==isSmall:"+isSmall()+"==mIsSwitchingState:"+mIsSwitchingState);
        ...
        if (!isSmall() && !mIsSwitchingState) {
        //if(mState != State.SMALL && !mIsSwitchingState){
            mInScrollArea = true;
    ...
    public boolean onExitScrollArea() {
        boolean result = false;
        android.util.Log.w("QsLog", "Workspace::onExitScrollArea()==isSmall:"+isSmall()+"==mInScrollArea:"+mInScrollArea);
    ...
    ArrayList<CellLayout> getWorkspaceAndHotseatCellLayouts() {
        ...
        for (int screen = 0; screen < screenCount; screen++) {
        	CellLayout child = (CellLayout) getChildAt(screen);
        	if(QS_SUPPORT_LOCK_CELLLAYOUT && child.isAttrsLocked())
        		continue;
            layouts.add(child);
        }
        if (mLauncher.getHotseat() != null) {
    ...
    ArrayList<CellLayoutChildren> getWorkspaceAndHotseatCellLayoutChildren() {
        ...
        for (int screen = 0; screen < screenCount; screen++) {
            ...
            childrenLayouts.add(child.getChildrenLayout());
        }
        if (mLauncher.getHotseat() != null) {
    ...
    void clearDropTargets() {
        ...
        for (CellLayoutChildren layout: childrenLayouts) {
            ...
            for (int j = 0; j < childCount; j++) {
                ...
                if (v instanceof DropTarget) {//if ((v instanceof DropTarget) && !(v instanceof AllAppsButtonDropTarget)) {
    ...
    void setFadeForOverScroll(float fade) {
        ...
        final ImageView dockDivider = null;// = (ImageView) (parent.findViewById(R.id.dock_divider));
